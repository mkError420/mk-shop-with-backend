...
const categoryConfigs = {
  electronics: {
    ...
    suggestions: ['smartphone', 'laptop', 'headphones', 'tablet', 'smart watch'],
    quickFilters: [
  ...
  computers: {
    color: 'purple',
    placeholder: 'Search for computers, components, or accessories...',
    suggestions: ['desktop computer', 'gaming laptop', 'monitor', 'keyboard', 'mouse'],
    quickFilters: [
      { label: 'All', value: 'computers', color: 'bg-gray-100 text-gray-700' },
      { label: 'Laptops', value: 'Laptops', color: 'bg-purple-100 text-purple-700' },
      { label: 'Desktops', value: 'Desktops', color: 'bg-blue-100 text-blue-700' },
      { label: 'Components', value: 'Components', color: 'bg-green-100 text-green-700' }
    ]
  },
  baby: {
    color: 'rose',
    placeholder: 'Search for baby products, brands, or essentials...',
    suggestions: ['baby stroller', 'diapers', 'baby clothes', 'baby toys', 'baby care'],
    quickFilters: [
  ...
  automotive: {
    color: 'slate',
    placeholder: 'Search for car parts, accessories, or brands...',
    suggestions: ['car battery', 'motor oil', 'car stereo', 'floor mats', 'car cover'],
    quickFilters: [
      { label: 'All', value: 'automotive', color: 'bg-gray-100 text-gray-700' },
      { label: 'Car Parts', value: 'Car Parts', color: 'bg-slate-100 text-slate-700' },
      { label: 'Accessories', value: 'Accessories', color: 'bg-blue-100 text-blue-700' },
      { label: 'Tools', value: 'Tools', color: 'bg-green-100 text-green-700' },
      { label: 'Electronics', value: 'Electronics', color: 'bg-purple-100 text-purple-700' }
    ]
  },
  music: {
    color: 'violet',
    placeholder: 'Search for instruments, audio equipment, or music gear...',
    suggestions: ['acoustic guitar', 'studio headphones', 'microphone', 'piano', 'music stand'],
    quickFilters: [
...
function updateCategoryPage(categoryName, config) {
  const filePath = path.join(__dirname, 'app', 'category', categoryName, 'page.tsx');
  ...
  // Replace search section with CategorySearch component
  const searchSectionRegex = /{\/\* Search Section \*\/[\s\S]*?<\/div>}/;
  const newSearchSection = `{/* Search Section */}
        <CategorySearch
  ...
  const searchLogicRegex = /const matchesSearch = searchTerm === '' \|\|[\s\S]*?product\.name\.toLowerCase\(\)\.includes\(searchTerm\.toLowerCase\(\)\)[\s\S]*?product\.category\.toLowerCase\(\)\.includes\(searchTerm\.toLowerCase\(\))/;
  ...
  content = content.replace(searchLogicRegex, newSearchLogic);

  // Enhanced sort options
  const sortOptionsRegex = /<option value='featured'>Featured<\/option>[\s\S]*?<option value='rating'>Highest Rated<\/option>/;
  ...
  // Enhanced sort logic
  const sortLogicRegex = /switch \(sortBy\) {[\s\S]*?default:[\s\S]*?return filtered[\s\S]*?}/;
  ...
  const depsRegex = /\}, \[searchTerm, sortBy, selectedRatings, selectedSizes, priceRange]\)/;
  content = content.replace(depsRegex, `}, [searchTerm, sortBy, selectedRatings, selectedSizes, priceRange, selectedCategory])`);
...
console.log('All category pages have been updated with enhanced search functionality!');
